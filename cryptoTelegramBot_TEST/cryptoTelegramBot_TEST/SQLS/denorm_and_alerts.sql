...
DELETE FROM price_denorm_t1a;

INSERT INTO price_denorm_t1a ( marketname,coin,exchange,exchange_last_price,exchange_last_price_in,volume )
...
COALESCE(All_Coins.volume,-1) as volume,
...
LEFT OUTER JOIN
(
SELECT marketname,coin,exchange,exchange_last_price,exchange_last_price_in,volume FROM price_denorm_t1a
) COM
...
SELECT
99999 AS rank,"-" AS id,"-" AS symbol,"-" AS name,MKTS.exchange,MKTS.marketname,MKTS.exchange_last_price,MKTS.exchange_last_price_in,"yes" AS is_new_market,MKTS.volume, unix_timestamp() AS created_at
FROM
(
